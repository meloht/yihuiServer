...
namespace Qiniu.IO
{
    ...
    {
        private static readonly int timeOut = 1000*3600;
        public static Encoding encoding = Config.Encoding;
        private static readonly int ReadLength = 1024*200;
        ...
        public static CallRet MultiPost(string url, NameValueCollection formData, string fileName, IWebProxy proxy = null, Action<int> onProgress = null)
        {
            ...
            using (FileStream fileStream = fileInfo.OpenRead())
            {
                ...
                int bytesRead = 0;
                long per = 0;
                long total = webRequest.ContentLength;
                int current = 0;
                onProgress?.Invoke(0);
                ...
                while ((bytesRead = postDataStream.Read(buffer, 0, buffer.Length)) != 0)
                {
                    ...
                    per += buffer.Length;
                    current = OnPercent(total, per, current, onProgress);
                }
        ...
        public static CallRet MultiPost(string url, NameValueCollection formData, System.IO.Stream inputStream, IWebProxy proxy = null, Action<int> onProgress = null)
        {
            string boundary = RandomBoundary();
            WebRequest webRequest = WebRequest.Create(url);
            webRequest.Timeout = timeOut;
        ...
        private static int OnPercent(long total, long index, int per, Action<int> onProgress)
        {
            if (onProgress != null)
            {
                long currentPercent = (index * 100) / total;
                if (currentPercent == per)
                    return per;
                onProgress.Invoke((int)currentPercent);
