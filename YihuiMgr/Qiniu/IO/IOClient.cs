...
namespace Qiniu.IO
{
    ...
    public class IOClient
    {
        ...
        public PutRet PutFile(string upToken, string key, string localFile, PutExtra extra)
        {
            ...
            try
            {
                CallRet callRet = MultiPart.MultiPost(Config.UP_HOST, formData, localFile, this.Proxy, OnProgress);
        ...
        public PutRet Put(string upToken, string key, System.IO.Stream putStream, PutExtra extra)
        {
            ...
            try
            {
                CallRet callRet = MultiPart.MultiPost(Config.UP_HOST, formData, putStream, null, OnProgress);
                ret = new PutRet(callRet);
                onPutFinished(ret);
                return ret;
            }
        ...
        {
            ProgressHandler?.Invoke(null, new ProgressEventArgs() { Progress = percent });
        }
